...
async function tryAutoLogin() {
  try {
    ...
    if (accounts.length > 0) {
      const result = await msalInstance.acquireTokenSilent({ scopes: CONFIG.scopes, account: accounts[0] });
...
function setStatus(html, isLoading = false) {
...
function renderFiles(files) {
  ...
  if (!files.length) {
    view.innerHTML = `<div class="empty"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg><p>No PowerPoint files found.<br>Try setting a SharePoint site URL.</p></div>`;
    return;
  }
...
async function openFile(file) {
  ...
  // Build slide placeholders based on estimated count
  ...
  // Try to get thumbnail for cover slide
  try {
...
function insertSlideNote(idx) {
  // Show helpful instructions since direct slide insertion requires server-side processing
  showToast(`Open file → copy slide ${idx} → paste here`);
  // Optionally open file in SharePoint for easy copy
  if (currentFile?.webUrl) {
    Office.context.ui.openBrowserWindow(currentFile.webUrl);
  }
}
...
async function downloadFile() {
  ...
  try {
    ...
    if (data['@microsoft.graph.downloadUrl']) {
      Office.context.ui.openBrowserWindow(data['@microsoft.graph.downloadUrl']);
    } else showToast('Download link unavailable');
  } catch(e) { showToast('Error: ' + e.message); }
}

function openInSharePoint() {
  if (currentFile?.webUrl) Office.context.ui.openBrowserWindow(currentFile.webUrl);
